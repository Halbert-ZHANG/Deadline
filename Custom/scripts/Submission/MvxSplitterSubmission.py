...
def __main__( *args ):
    ...
    scriptDialog.SetTitle( "Submit MvxSplitter Job To Deadline" )
    scriptDialog.SetIcon(scriptDialog.GetIcon( 'MvxSplitter' ))

    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator1", "SeparatorControl", "Job Description", 0, 0, colSpan=2 )

    scriptDialog.AddControlToGrid( "NameLabel", "LabelControl", "Job Name", 1, 0, "The name of your job. This is optional, and if left blank, it will default to 'Splitter'.", False )
    scriptDialog.AddControlToGrid( "NameBox", "TextControl", "Splitter", 1, 1 )
    ...
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "MvxSplitter Options", 0, 0, colSpan=4 )

    scriptDialog.AddControlToGrid( "SceneLabel", "LabelControl", "Mvx File", 1, 0, "The TVPaint script to be rendered.", False )
    scriptDialog.AddSelectionControlToGrid( "SceneBox", "FileBrowserControl", "", "Mvx Files (*.mvx)", 1, 1, colSpan=3 )
    ...
    scriptDialog.AddControlToGrid( "FramesLabel", "LabelControl", "Frame Step", 3, 0, "The list of frames for the normal job.", False )
    ...
    settings = ("DepartmentBox","CategoryBox","PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","MachineLimitBox","IsBlacklistBox","MachineListBox","LimitGroupBox","FramesBox", "SceneBox")
...
def GetSettingsFilename():
    return Path.Combine( GetDeadlineSettingsPath(), "MvxSplitterSettings.ini" )
...
def SubmitButtonPressed(*args):
    ...
    jobInfoFilename = Path.Combine( GetDeadlineTempPath(), "mvxsplitter_job_info.job" )
    writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
    writer.WriteLine( "Plugin=MvxSplitter" )
    ...
    pluginInfoFilename = Path.Combine( GetDeadlineTempPath(), "mvxsplitter_plugin_info.job" )
    writer = StreamWriter( pluginInfoFilename, False, Encoding.Unicode )

    writer.WriteLine("MvxFile=%s" % scriptDialog.GetValue("SceneBox"))

    args = '-i <QUOTE>%s<QUOTE>' % scriptDialog.GetValue("SceneBox")
    args += ' -s <QUOTE>%s<QUOTE>' % scriptDialog.GetValue("FramesBox")
