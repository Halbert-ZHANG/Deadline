...
def __main__( *args ):
    ...
    scriptDialog.SetTitle( "Submit MvxXmlGraphRunner Job To Deadline" )
    scriptDialog.SetIcon(scriptDialog.GetIcon( 'MvxXmlGraphRunner' ))

    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator1", "SeparatorControl", "Job Description", 0, 0, colSpan=2 )

    scriptDialog.AddControlToGrid( "NameLabel", "LabelControl", "Job Name", 1, 0, "The name of your job. This is optional, and if left blank, it will default to 'MvxXmlGraphRunner'.", False )
    scriptDialog.AddControlToGrid( "NameBox", "TextControl", "MvxXmlGraphRunner", 1, 1 )
    ...
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "MvxXmlGraphRunner Options", 0, 0, colSpan=4 )

    scriptDialog.AddControlToGrid( "SceneLabel", "LabelControl", "Mvx File", 1, 0, "The Mvx files to be rendered.", False )
    scriptDialog.AddSelectionControlToGrid( "SceneBox", "MultiFileBrowserControl", "", "Mvx Files (*.mvx)", 1, 1, colSpan=3 )
    ...
    scriptDialog.AddControlToGrid( "GraphLabel", "LabelControl", "Graph File", 3, 0, "The script to be rendered.", False )
    scriptDialog.AddSelectionControlToGrid( "GraphBox", "FileBrowserControl", "", "Graph Files (*.xml)", 3, 1, colSpan=3 )

    scriptDialog.AddControlToGrid( "ArgumentsLabel", "LabelControl", "Arguments(optional)", 4, 0, "The TVPaint script to be rendered.", False )
    scriptDialog.AddSelectionControlToGrid( "ArgumentsBox", "TextControl", "--start 0 ", "", 4, 1, colSpan=3 )
    ...
    "SceneBox","OutputBox","GraphBox","ArgumentsBox")
...
def GetSettingsFilename():
    return Path.Combine( GetDeadlineSettingsPath(), "MvxXmlGraphRunnerSettings.ini" )
...
def SubmitButtonPressed(*args):
    ...
    if( len( sceneFiles ) == 0 ):
        scriptDialog.ShowMessageBox( "No Mvx file specified", "Error" )
        return
    
    for sceneFile in sceneFiles:
        if( not File.Exists( sceneFile ) ):
            scriptDialog.ShowMessageBox( "Mvx file %s does not exist" % sceneFile, "Error" )
            return

    if not os.path.exists(scriptDialog.GetValue("OutputBox")):
    ...
    for sceneFile in sceneFiles:
        ...
        jobInfoFilename = Path.Combine( GetDeadlineTempPath(), "mvxxmlgraphrunner_job_info.job" )
        writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
        writer.WriteLine( "Plugin=MvxXmlGraphRunner" )
        ...
        if len(sceneFiles) > 1:
            ...
            #writer.WriteLine( "BatchName=MvxXmlGraphRunner\n" )

        writer.Close()

        # Create plugin info file.
        pluginInfoFilename = Path.Combine( GetDeadlineTempPath(), "mvxxmlgraphrunner_plugin_info.job" )
        writer = StreamWriter( pluginInfoFilename, False, Encoding.Unicode )

        #writer.WriteLine("MvxFile=%s" % scriptDialog.GetValue("SceneBox"))

        args = '<QUOTE>%s<QUOTE>' % scriptDialog.GetValue("GraphBox")
        args += ' --input <QUOTE>%s<QUOTE>' % sceneFile
        args += ' --output <QUOTE>%s<QUOTE>' % os.path.join(scriptDialog.GetValue("OutputBox"), os.path.basename(sceneFile))
        args += ' %s' % scriptDialog.GetValue("ArgumentsBox")
